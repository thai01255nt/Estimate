...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F acc_dot = attitude.Rotate_BtoI(accel);
  predictedState(0) = curState(0) + curState(3)*dt;
  predictedState(1) = curState(1) + curState(4)*dt;
  predictedState(2) = curState(2) + curState(5)*dt;
  predictedState(3) = curState(3) + acc_dot.x*dt;
  predictedState(4) = curState(4) + acc_dot.y*dt;
  predictedState(5) = curState(5) + (acc_dot.z - CONST_GRAVITY)*dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 0) = - c_q * s_r;
  RbgPrime(0, 1) = - s_p * s_q* s_r - c_r * c_p;
  RbgPrime(0, 2) = s_p * c_r - s_q * c_p*s_r;
  RbgPrime(1, 0) = c_r * c_q;
  RbgPrime(1, 1) = s_p * s_q*c_r - c_p * s_r;
  RbgPrime(1, 2) = s_p * s_r + s_q * c_r*c_p;
  RbgPrime(2, 0) = 0;
  RbgPrime(2, 1) = 0;
  RbgPrime(2, 2) = 0;
